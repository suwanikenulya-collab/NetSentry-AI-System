...
const SUMMARY_CARDS = [
  {
    label: "Total users",
    value: "—",
    description: "Awaiting user database",
    icon: Users,
...
export function UserManagementPage() {
  const [activeFilter, setActiveFilter] =
    useState<(typeof USER_FILTERS)[number]>("All users");
      ...
      <header className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-3xl text-muted">
            Manage user accounts, account status and access roles
            within NetSentry.
          </p>
        </div>
        ...
        <button
          type="button"
          disabled
          title="User creation requires the authentication backend"
          className="inline-flex cursor-not-allowed items-center justify-center gap-2 rounded-lg bg-primary px-4 py-2.5 text-sm font-medium text-primary-foreground opacity-50"
        >
      ...
      <section className="overflow-hidden rounded-2xl border border-border bg-surface">
          ...
          <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-muted">
                Search accounts and filter users by role or status.
              </p>
            </div>
        ...
        <div className="flex min-h-96 flex-col items-center justify-center border-t border-border p-8 text-center">
          ...
          <p className="mt-2 max-w-lg text-sm leading-6 text-muted">
            User accounts will appear here after database-backed
